...
| `snapshots/` | frozen copies of the run-log index and the latest ablations-measurement doc, dated |
...
| `data/all_metrics_long.parquet` | tidy/long: one row per (run, step, metric). **Not committed** — regenerated by `the internal refresh pipeline (not included)`. |
...
```python
import pandas as pd
df = pd.read_parquet("data/all_metrics_long.parquet")  # regenerated by the internal refresh pipeline (not included) if missing
...
- Repo runlog: `internal notes (not included)`
- Latest ablations measurement: `../../docs/eval/<latest>-ablations_measurment.md`
- MMD-ratio analysis: `internal notes (not included)`
